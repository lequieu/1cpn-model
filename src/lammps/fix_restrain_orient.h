...
FixStyle(restrain_orient,FixRestrainOrient)

#else

#ifndef LMP_FIX_RESTRAIN_ORIENT_H
#define LMP_FIX_RESTRAIN_ORIENT_H

#include "fix.h"

namespace LAMMPS_NS {

class FixRestrainOrient : public Fix {
 public:
  FixRestrainOrient(class LAMMPS *, int, char **);
  ~FixRestrainOrient();
  int setmask();
  void init();
  void setup(int);
